...
class Rotator:
    ...
    @jit
    def spot_coords(self, time=None, t0=0):
        contrast = self.spectrum / self.phot
        ...
        phase = jnp.expand_dims(2 * np.pi * (time - t0) / self.P_rot, [1, 2, 3])
        lon = jnp.expand_dims(self.lon, [0, 2, 3])
        lat = jnp.expand_dims(self.lat, [0, 2, 3])
        rad = jnp.expand_dims(self.rad, [0, 2, 3])
        contrast = jnp.expand_dims(contrast, [0, 3])
    ...
    def transit_model(self, t0, period, rp, a, inclination, f_S, omega=np.pi / 2, ecc=0):
        ...
        def time_step(
            ...
            spot_position_x=spot_position_x, spot_position_z=spot_position_z, major_axis=major_axis,
            ...
            def spot_step(
                ...
                spot_position_x=spot_position_x[j, :, 0, 0], spot_position_z=spot_position_z[j, :, 0, 0],
        ...
        occultation = (
                (1 - contrast[None, ..., None]) * occultation[..., None, None] * rp ** 2
        )
